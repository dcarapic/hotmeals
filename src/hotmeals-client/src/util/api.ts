...
else serverUrl = "http://localhost:6010/";
...
export async function userAuthenticate(
  abort?: AbortSignal
): Promise<ServerResponse<AuthenticateResponse>> {
...
export async function userLogin(
  ...
  let response = await request<LoginRequest, LoginResponse>(
    "Login",
    "api/user/login",
    "POST",
    req,
  ...
  if (response.ok && response.result)
    jwt.setJWTToken(response.result.jwtToken, response.result.expiresInSeconds);
  return response;
}
...
export async function ordersFetchActive(
  page: number,
  abort?: AbortSignal
...
async function request<TReq, TResp extends APIResponse>(
  ...
  if (result.ok) console.log(`%c ... ${result.statusCode}`, "color: darkblue");
    ...
    console.log(
      `%c ... ${result.statusCode} / ${result.errorDetails}`,
      "color: darkblue"
    );
...
async function updateResultFromBody<T>(
...
async function extractErrorDetails(response: Response): Promise<string> {
  ...
  try {
    ...
    if (response.status === 401)
      errorMessage =
        "You are not logged in or your credentials have expired. Please login again.";
    if (response.status === 403)
      errorMessage = "You do not have permission to access requested resource.";
